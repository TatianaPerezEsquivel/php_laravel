   ...
   <div class="row">
    <div class="col-md-6">
        ...
        <select name="estudiante_id" class="form-control" required>
         ...
         <option value="{{ $estudiante->id}}" @if($estudiante->id == $estudianteGrupo->estudiante_id) selected @endif> {{$estudiante->nombre}} {{$estudiante->apellido}}</option>
         @endforeach
        </select>
    </div>
   </div>
   <div  class="row">
    <div class="col-md-4">
        ...
        <select name="grupo_id" class="form-control" required>
          ...
          <option value="{{ $grupo->id}}" @if($grupo->id == $estudianteGrupo->grupo_id) selected @endif> {{ $grupo->nombre}}</option>
          @endforeach
         </select>
    </div>
   </div>
